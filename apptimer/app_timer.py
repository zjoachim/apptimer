...
def get_active_window_process_name():
    ...
    # Tier 1：完整权限（PROCESS_QUERY_INFORMATION | PROCESS_VM_READ）
    handle = kernel32.OpenProcess(0x0400 | 0x0010, False, pid_val)
    if handle == 0:
        # Tier 2：受限查询权限（PROCESS_QUERY_LIMITED_INFORMATION），反作弊更可能放过
        ...
        if handle != 0:
            ...
            if exe_path2:
                name2 = Path(exe_path2).name.lower()
                return name2, exe_path2

        # Tier 3：CreateToolhelp32Snapshot 枚举进程，完全不需要 OpenProcess
...
class AppTimerUI:
    ...
    def _build_ui(self):
        ...
        for text, cmd in [("退出", self._do_exit), ("导出 PDF", self._export_pdf),
                          ("导出 CSV", self._export_csv), ("打开文件夹", self._open_folder)]:
            ttk.Button(self.header, text=text, command=cmd).pack(side=tk.RIGHT, padx=2)
        self.dark_btn = ttk.Button(self.header, text="深色模式", command=self._toggle_dark_mode)
        self.dark_btn.pack(side=tk.RIGHT, padx=(12, 2))
    ...
    def _build_today_tab(self, nb):
        ...
        for w in (self.canvas, self.list_frame):
            w.bind("<MouseWheel>", lambda e: self.canvas.yview_scroll(-1 * (e.delta // 120), "units"))
    ...
    def _build_cumulative_tab(self, nb):
        ...
        # 图表切换
        chart_bar = ttk.Frame(tab)
        chart_bar.pack(fill=tk.X, padx=20, pady=(4, 0))
        ttk.Label(chart_bar, text="图表:").pack(side=tk.LEFT)
        self.cum_chart_type = tk.StringVar(value="bar")
        ttk.Radiobutton(chart_bar, text="柱状图", variable=self.cum_chart_type, value="bar",
                        command=self.refresh).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(chart_bar, text="饼图", variable=self.cum_chart_type, value="pie",
                        command=self.refresh).pack(side=tk.LEFT, padx=5)

        # 累计图表
        self.chart_cum_canvas = tk.Canvas(tab, bg=self.colors["card"], highlightthickness=0, height=240)
        self.chart_cum_canvas.pack(fill=tk.X, padx=20, pady=4)
        ...
        for w in (self.cum_canvas, self.cum_list):
            w.bind("<MouseWheel>", lambda e: self.cum_canvas.yview_scroll(-1 * (e.delta // 120), "units"))
    ...
    def _build_categories_tab(self, nb):
        ...
        ttk.Button(btn_row, text="设置分类", command=self._set_category).pack(side=tk.LEFT, padx=(0, 3))
        ttk.Button(btn_row, text="删除选中", command=self._del_category).pack(side=tk.LEFT)

        sel = ttk.Frame(left)
        sel.pack(fill=tk.X, pady=(3, 5))
        self.cat_app_var = tk.StringVar()
        self.cat_app_combo = ttk.Combobox(sel, textvariable=self.cat_app_var, width=22, state="readonly")
        self.cat_app_combo.pack(side=tk.LEFT, padx=(0, 3))

        self.cat_val_var = tk.StringVar(value="未分类")
        cat_combo = ttk.Combobox(sel, textvariable=self.cat_val_var, width=12, state="readonly",
        ...
        goal_btn_row = ttk.Frame(right)
        goal_btn_row.pack(fill=tk.X, pady=(5, 0))
        ttk.Button(goal_btn_row, text="设定目标", command=self._set_goal).pack(side=tk.LEFT, padx=(0, 3))
        ttk.Button(goal_btn_row, text="删除选中", command=self._del_goal).pack(side=tk.LEFT)

        goal_sel = ttk.Frame(right)
        goal_sel.pack(fill=tk.X, pady=(3, 5))
        ...
        self.goal_app_combo.pack(side=tk.LEFT, padx=(0, 3))

        self.goal_min_var = tk.IntVar(value=60)
        ttk.Spinbox(goal_sel, from_=1, to=480, textvariable=self.goal_min_var, width=4).pack(side=tk.LEFT, padx=3)
    ...
    def _bind_scroll_recursive(self, widget, target_canvas):
        """递归给 widget 及其所有子控件绑定 MouseWheel，解决 Tk 不冒泡的问题"""
        widget.bind("<MouseWheel>", lambda e, c=target_canvas: c.yview_scroll(-1 * (e.delta // 120), "units"))
        for child in widget.winfo_children():
            self._bind_scroll_recursive(child, target_canvas)
    ...
    def _refresh_ui(self):
        ...
        if not usage:
            ...
            self._bind_scroll_recursive(self.list_frame, self.canvas)
        ...
        if not cum_usage:
            ...
            self._bind_scroll_recursive(self.cum_list, self.cum_canvas)

        # ── 图表 ──
        self._draw_chart()
        self._draw_cum_chart()
    ...
    def _draw_cum_chart(self):
        """累计统计 tab 专属图表，数据源固定为累计数据"""
        canvas = self.chart_cum_canvas
        canvas.delete("all")
        c = self.colors
        ...
        if not data:
            ...
            canvas.create_text(w // 2, 110, text="暂无累计数据", fill=c["text_secondary"],
        ...
        w = (lambda v: v if v > 50 else 700)(canvas.winfo_width())
        h = (lambda v: v if v > 50 else 240)(canvas.winfo_height())
        colors_palette = ["#2196F3", "#4CAF50", "#FF9800", "#9C27B0", "#00BCD4",
                          "#E91E63", "#FF5722", "#3F51B5", "#009688", "#795548"]

        if self.cum_chart_type.get() == "pie":
            ...
            col_w = 180
            cols = 1 if len(legend_items) <= 5 else 2
            base_y = max(h - 10 - (len(legend_items) + cols - 1) // cols * 20, 10)
            for i, (app, sec) in enumerate(legend_items):
                col = i // 5
                row = i % 5
                x = 10 + col * col_w
            ...
            margin_l, margin_r, margin_t, margin_b = 70, 20, 20, 50
            chart_w = w - margin_l - margin_r
            chart_h = h - margin_t - margin_b
            max_val = sorted_data[0][1] if sorted_data else 1
            bar_w = min(chart_w / len(sorted_data) - 10, 60)
            ...
            for i, (app, sec) in enumerate(sorted_data):
                ...
                canvas.create_rectangle(x + 5, y, x + 5 + bar_w, margin_t + chart_h,
                                        fill=color, outline="")
                val_text = fmt_duration(sec)
                canvas.create_text(x + 5 + bar_w / 2, y - 8, text=val_text,
                                   fill=c["fg"], font=("Microsoft YaHei", 8), angle=0)
                short_app = app[:8] + ".." if len(app) > 8 else app
                canvas.create_text(x + 5 + bar_w / 2, margin_t + chart_h + 15, text=short_app,
                                   fill=c["fg"], font=("Microsoft YaHei", 8), angle=45, anchor=tk.W)
    ...
    def _del_category(self):
        ...
        if sel:
            app = self.cat_tree.item(sel[0], "values")[0]
            with self.tracker.data_lock:
                if app in self.tracker.categories:
                    del self.tracker.categories[app]
            self.tracker.save_categories()
            self._refresh_cat_tree()
            self.refresh()
    ...
    def _del_goal(self):
        ...
        if sel:
            app = self.goal_tree.item(sel[0], "values")[0]
            with self.tracker.data_lock:
    ...
    def _init_tray(self):
        try:
            ...
            logging.warning("托盘图标初始化失败，将以后台模式运行")
            self.tray_icon = None
    ...
    def _uninstall(self):
        ...
        if data_path.exists() and messagebox.askyesno(
            "保留数据？",
            f"是否删除所有使用记录？\n\n数据目录: {data_dir}\n\n选「是」将永久删除所有历史数据\n选「否」保留数据",
            icon="warning"
        ):
...
def main():
    # PyInstaller multiprocessing 支持（必须最先调用，防止子进程重复启动）
    try:
    ...
    # 单实例检查：命名 mutex 防止重复打开
    import ctypes.wintypes
    _mutex = ctypes.windll.kernel32.CreateMutexW(None, False, "Local\\AppTimer_SingleInstance")
    _le = ctypes.windll.kernel32.GetLastError()
    if _le == 183:  # ERROR_ALREADY_EXISTS
        os._exit(0)
